...
### [x] Step 2.3: 实现 HeroDisplayView (大色卡预览区)
...
### [x] Step 2.4: 实现 OccasionSelectorView & PaletteSelectorView (横向滚动列表)
...
### [x] Step 2.5: 实现 BottomActionsView (底部操作栏)
...
### [x] Step 4.1: 实现 SettingsPanelView (侧滑设置面板)
...
## Phase 5: MVP 核心功能扩展 (MVP Features Extension)

### [x] Step 5.1: 本地数据持久化 (Persistence)
- 实现 `TodayOutfitState` 的 Preferences 存储。
- 跨日启动时自动快照“昨日”配色，并恢复今日状态。

### [x] Step 5.2: 选色路径扩展 (Extended Selection Paths)
- 实现“随机”（仅在安全集内）、“昨日”配色的读取与应用。
- 实现“精调”面板（Fine-tune Sheet），允许用户基于当前套组覆盖自定义 HEX 颜色。

### [x] Step 5.3: 出图与分享 (Export & Share)
- 接入 ImageKit，实现离屏渲染（PixelMap），支持多套基础版式模板（如横条、竖条、三分）。
- 解决底层 Skia 引擎的 R/B 通道反转问题（按 BGRA 写入）。
- 接入 MediaLibrary，实现通过系统弹窗（`showAssetsCreationDialog`）安全保存图片到相册。
- 接入 Share Kit，实现原生分享面板。

### [x] Step 5.4: 桌面服务卡片 (Form Card)
- 创建中卡（2x4）服务卡片。
- 实现卡片与主应用状态同源（读取 Preferences），展示色条与场合。
- 点击卡片直达主应用。

### [x] Step 5.5: 辅助功能与无障碍 (Accessibility)
- 接入“减少动效”开关，关闭所有非必要 UI 动画。
- 接入“轻噪点叠层”开关，为 Hero 区域添加质感。
- 主题跟随系统策略展示。
